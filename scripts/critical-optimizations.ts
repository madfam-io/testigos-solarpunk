...
`;
  ...
  try {
    ...
    if (logoStats.size > 50000) {
      // If larger than 50KB
      console.log(
        `   ⚠️  Logo is ${(logoStats.size / 1024).toFixed(1)}KB - needs manual optimization`
      );
      console.log(
        '   💡 Use: npx @squoosh/cli --webp auto --resize {width:512} public/images/logo.png'
      );
    }
  } catch {
