...
async function testKeyboardNavigation(
  url: string
): Promise<Array<{ element: string; issue: string }>> {
  ...
  try {
    ...
    for (const element of textElements.slice(0, 50)) {
      ...
      if (contrast && contrast.bg && contrast.fg) {
        ...
        const isDarkBg =
          contrast.bg.includes('0, 0, 0') || contrast.bg.includes('dark');
        const isDarkFg =
          contrast.fg.includes('0, 0, 0') || contrast.fg.includes('dark');
...
async function runAccessibilityTests(): Promise<AccessibilityTestResult[]> {
  ...
  for (const { lang, theme } of testMatrix) {
    for (const page of pagesToTest) {
      ...
      console.log(
        chalk.gray(`  Testing ${lang}${page} with ${theme} theme...`)
      );
...
function generateReport(report: AccessibilityReport): void {
  ...
  console.log(
    `  Tests passed: ${report.summary.testsPassed}/${report.summary.testsRun}`
  );
  ...
  pageResults.forEach((stats, page) => {
    ...
    console.log(
      `  ${status} ${page}: ${stats.passed}/${stats.total} combinations passed`
    );
  });
  ...
  if (report.contrastIssues.length > 0) {
    ...
    report.contrastIssues.slice(0, 5).forEach((issue) => {
      console.log(
        `  - ${issue.selector}: ratio ${issue.ratio} (required: ${issue.required})`
      );
    });
  }
  ...
  if (
    ...
    report.summary.wcagCompliance === 'AAA'
  ) {
    console.log(
...
async function saveDetailedReport(report: AccessibilityReport): Promise<void> {
  const reportPath = path.join(
    process.cwd(),
    'reports',
    'accessibility-health-check.json'
  );
...
async function main() {
  try {
    ...
    console.error(
      chalk.red('\n❌ Error running accessibility health check:'),
      error
    );
    process.exit(1);
  }
