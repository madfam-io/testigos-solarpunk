...
async function runHealthCheck(
  ...
  try {
    ...
    const reportName = script.replace('scripts/health-check-', '').replace('.ts', '');
    const reportPath = path.join(process.cwd(), 'reports', `${reportName}-health-check.json`);
    ...
    if (await fs.pathExists(reportPath)) {
      ...
      switch (reportName) {
        ...
        case 'performance':
          ...
          criticalIssues = report.failedTests?.filter((t: any) => 
            t.metric.includes('Score')).length || 0;
...
async function generateConsolidatedReport(results: HealthCheckResult[]): Promise<OverallHealthReport> {
    ...
    if (check.status === 'failed') {
      switch (check.name) {
        case 'i18n':
          ...
          break;
        case 'Theme System':
          recommendations.push('Complete theme variable definitions and fix contrast issues');
          break;
        case 'Performance':
          recommendations.push('Optimize bundle sizes and improve Core Web Vitals');
          break;
        case 'Accessibility':
...
function displayReport(report: OverallHealthReport): void {
  ...
  const scoreColor = 
    report.healthScore >= 90 ? chalk.green :
    report.healthScore >= 70 ? chalk.yellow :
    chalk.red;
  
  console.log(chalk.cyan('\n📊 Overall Health Score: ') + scoreColor.bold(`${report.healthScore}/100`));
  console.log(chalk.cyan('📋 Status: ') + getStatusEmoji(report.status) + ' ' + chalk.bold(report.status.toUpperCase()));
  console.log(chalk.cyan('🚀 Production Ready: ') + (report.readyForProduction ? chalk.green('YES') : chalk.red('NO')));
  ...
  report.checks.forEach(check => {
    ...
    const statusColor = 
      check.status === 'passed' ? chalk.green :
      check.status === 'warning' ? chalk.yellow :
      chalk.red;
    
    console.log(`  ${statusEmoji} ${chalk.bold(check.name)}: ${statusColor(check.status.toUpperCase())}`);
  ...
  if (report.status === 'excellent') {
    ...
    console.log(chalk.yellow('Address the warnings to achieve excellent status.\n'));
...
async function saveConsolidatedReport(report: OverallHealthReport): Promise<void> {
  const reportPath = path.join(process.cwd(), 'reports', 'comprehensive-health-report.json');
  await fs.ensureDir(path.dirname(reportPath));
  await fs.writeJSON(reportPath, report, { spaces: 2 });
  
  // Also save an HTML summary
  const htmlPath = path.join(process.cwd(), 'reports', 'health-check-summary.html');
...
function generateHTMLSummary(report: OverallHealthReport): string {
  const statusColor = 
    report.status === 'excellent' ? '#22c55e' :
    report.status === 'good' ? '#f59e0b' :
    report.status === 'needs-attention' ? '#f97316' :
    '#ef4444';
...
async function main() {
  ...
  console.log(chalk.gray('This will run all health checks and may take several minutes.\n'));
  ...
  try {
    ...
    try {
      execSync('curl -s http://localhost:4321 > /dev/null', { stdio: 'ignore' });
      devServerRunning = true;
    } catch {
      console.log(chalk.yellow('⚠️  Dev server not running. Some checks will be skipped.\n'));
    }
    ...
    console.error(chalk.red('\n❌ Error running comprehensive health check:'), error);
    process.exit(1);
  }
}
