...
async function runPerformanceTests(): Promise<PerformanceTestResult[]> {
  ...
  for (const { lang, theme } of testMatrix) {
    ...
    try {
      ...
      if (lhr) {
        const result: PerformanceTestResult = {
          ...
          resources: {
            ...
            jsSize: lhr.audits['total-byte-weight'].details?.items?.find(
              (item: any) => item.label === 'Script'
            )?.totalBytes || 0,
            cssSize: lhr.audits['total-byte-weight'].details?.items?.find(
              (item: any) => item.label === 'Stylesheet'
            )?.totalBytes || 0,
            imageSize: lhr.audits['total-byte-weight'].details?.items?.find(
              (item: any) => item.label === 'Image'
            )?.totalBytes || 0,
            fontSize: lhr.audits['total-byte-weight'].details?.items?.find(
      ...
      console.error(chalk.red(`    Failed to test ${lang}/${theme}: ${error.message}`));
    }
  }
...
async function checkBundleSizes(): Promise<Array<{ file: string; size: number; threshold: number }>> {
  ...
  try {
    ...
    const jsFiles = await fs.readdir(path.join(distPath, '_assets'))
    ...
    for (const file of jsFiles) {
      ...
      if (stats.size > 200000) { // 200KB threshold for individual JS files
        issues.push({
          file: file,
          size: stats.size,
          threshold: 200000
...
function analyzeResults(results: PerformanceTestResult[]): PerformanceIssues {
  ...
  results.forEach(result => {
    ...
    // Check scores
    Object.entries(performanceThresholds.scores).forEach(([metric, threshold]) => {
    ...
    // Check metrics
    Object.entries(performanceThresholds.metrics).forEach(([metric, threshold]) => {
    ...
    // Check resources
    if (result.resources.totalSize > performanceThresholds.resources.totalSize) {
      issues.failedTests.push({
        combination,
  ...
  // Calculate summary
  if (results.length > 0) {
    issues.summary.testsPassed = results.length - 
    ...
    issues.summary.averageLCP = Math.round(
      results.reduce((sum, r) => sum + r.metrics.largestContentfulPaint, 0) / results.length
    );
    
    issues.summary.averageFCP = Math.round(
      results.reduce((sum, r) => sum + r.metrics.firstContentfulPaint, 0) / results.length
    );
...
function generateReport(issues: PerformanceIssues): void {
  ...
  console.log(`  Tests passed: ${issues.summary.testsPassed}/${issues.summary.testsRun}`);
  console.log(`  Average performance score: ${issues.summary.averagePerformance}/100`);
  console.log(`  Average LCP: ${issues.summary.averageLCP}ms`);
  console.log(`  Average FCP: ${issues.summary.averageFCP}ms`);
  console.log(`  Max bundle size: ${(issues.summary.totalBundleSize / 1024 / 1024).toFixed(2)}MB`);
  
  // Test results by combination
  console.log(chalk.cyan('\n🎯 Results by Language/Theme:'));
  issues.testResults.forEach(result => {
    const status = issues.failedTests.some(f => 
      f.combination === `${result.language}-${result.theme}`
    ) ? '❌' : '✅';
    
    console.log(`  ${status} ${result.language.toUpperCase()}-${result.theme}: ` +
  ...
  if (issues.failedTests.length > 0) {
    ...
    issues.failedTests.slice(0, 10).forEach(failure => {
      console.log(`  - ${failure.combination} | ${failure.metric}: ${failure.value} (threshold: ${failure.threshold})`);
    });
  ...
  if (issues.bundleSizeIssues.length > 0) {
    ...
    issues.bundleSizeIssues.forEach(issue => {
      console.log(`  - ${issue.file}: ${(issue.size / 1024).toFixed(2)}KB (threshold: ${(issue.threshold / 1024).toFixed(2)}KB)`);
    });
  }
  ...
  if (issues.failedTests.length === 0 && issues.bundleSizeIssues.length === 0) {
    ...
    console.log(chalk.yellow('Minor performance issues found that should be addressed.'));
  } else {
    console.log(chalk.red.bold('\n❌ Performance Health Status: NEEDS ATTENTION'));
    console.log(chalk.red('Multiple performance issues found that require immediate attention.'));
  }
...
async function saveDetailedReport(issues: PerformanceIssues): Promise<void> {
  const reportPath = path.join(process.cwd(), 'reports', 'performance-health-check.json');
...
async function main() {
  try {
    ...
    console.error(chalk.red('\n❌ Error running performance health check:'), error);
    process.exit(1);
  }
}
