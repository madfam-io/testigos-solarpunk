...
interface ThemeIssues {
  ...
  performanceIssues: Array<{ metric: string; value: number; threshold: number }>;
...
const contrastRequirements = [
  { bg: '--bg-primary', fg: '--text-primary', minRatio: 7, context: 'Main content' },
  { bg: '--bg-secondary', fg: '--text-primary', minRatio: 7, context: 'Cards/sections' },
  { bg: '--bg-tertiary', fg: '--text-secondary', minRatio: 4.5, context: 'Subtle elements' },
  { bg: '--color-eco-green', fg: '--text-on-accent', minRatio: 4.5, context: 'Green buttons' },
  { bg: '--color-solar-yellow', fg: '--text-primary', minRatio: 4.5, context: 'Yellow accents' },
  { bg: '--color-comedy-purple', fg: '--text-on-accent', minRatio: 4.5, context: 'Purple elements' }
];

async function checkThemeVariables(): Promise<Pick<ThemeIssues, 'missingVariables'>> {
  ...
  for (const file of themeCSSFiles) {
    ...
    const themeName = file.includes('dark') ? 'dark' : 
                     file.includes('light') ? 'light' : 'base';
...
async function checkContrastRatios(): Promise<Pick<ThemeIssues, 'contrastIssues'>> {
  ...
  console.log(chalk.gray('  (Contrast ratio checking requires manual verification)'));
  
  return issues;
}

async function checkHardcodedColors(): Promise<Pick<ThemeIssues, 'hardcodedColors'>> {
...
async function checkThemePerformance(): Promise<Pick<ThemeIssues, 'performanceIssues'>> {
  ...
  try {
    ...
    console.log(chalk.gray('  (Performance testing requires dev server running)'));
  } finally {
    await browser.close();
  }
...
async function checkMagazineAesthetic(): Promise<Pick<ThemeIssues, 'aestheticIssues'>> {
  console.log(chalk.yellow('Checking magazine cutout aesthetic preservation...'));
  ...
  if (aestheticClassUsage < requiredClasses.length * 2) {
    issues.aestheticIssues.push('Limited use of magazine cutout aesthetic classes');
  }
...
function generateReport(issues: ThemeIssues): void {
  ...
  if (issues.contrastIssues.length > 0) {
    ...
    issues.contrastIssues.forEach(issue => {
      console.log(`  - ${issue.context}: ${issue.ratio} (required: ${issue.required})`);
    });
  }
  ...
  if (issues.performanceIssues.length > 0) {
    ...
    issues.performanceIssues.forEach(issue => {
      console.log(`  - ${issue.metric}: ${issue.value}ms (threshold: ${issue.threshold}ms)`);
    });
  }
  ...
  if (issues.summary.issuesFound === 0) {
    ...
    console.log(chalk.red('Multiple issues found that require immediate attention.'));
  }
...
async function saveDetailedReport(issues: ThemeIssues): Promise<void> {
  const reportPath = path.join(process.cwd(), 'reports', 'theme-health-check.json');
    ...
    const allIssues: ThemeIssues = {
      ...
      summary: {
        ...
        variableCoverage: Math.round(
          ((requiredVariables.length - variableIssues.missingVariables.length) / requiredVariables.length) * 100
        ),
        contrastCompliance: contrastIssues.contrastIssues.length === 0 ? 100 : 0,
        performanceScore: performanceIssues.performanceIssues.length === 0 ? 100 : 50,
        issuesFound: 
