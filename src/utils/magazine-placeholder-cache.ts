...
export class MagazinePlaceholderCache {
  ...
  static get(key: string): GeneratedPlaceholder | null {
    ...
    const now = Date.now();
    const isExpired = now - entry.timestamp > magazinePlaceholderConfig.performance.cacheDuration;
      ...
      if (!this.get(config.key)) {
        try {
          ...
          const placeholder: GeneratedPlaceholder = {
            url: this.generateDefaultSVG(config.type, config.width, config.height),
   ...
   */
  private static generateDefaultSVG(type: string, width: number, height: number): string {
    const messages = {
      character: 'PERSONA',
      sketch: 'SKETCH',
      hero: 'TESTIGOS',
  ...
  static cleanExpired(): number {
    ...
    for (const [key, entry] of this.cache) {
      if (now - entry.timestamp > magazinePlaceholderConfig.performance.cacheDuration) {
        expired.push(key);
      }
    }
